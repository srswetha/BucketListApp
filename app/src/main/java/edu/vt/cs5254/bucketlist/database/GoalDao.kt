...
interface GoalDao {
    ...
    @Query("SELECT * FROM goal g LEFT JOIN goal_note n ON g.id = n.goalId ORDER BY g.lastUpdated DESC")
    ...
    suspend fun getGoalAndNotes(id: UUID): Goal {
        return internalGetGoal(id).apply {
            notes = internalGetNotesForGoal(id)
        }
    }
    @Transaction
